...
pub fn build(b: *std.Build) void {
    ...
    // WASM step (browser target)
    const wasm_step = b.step("wasm", "Build the wasm32-freestanding artifact (lib-wasm.zig)");
    const wasm_optimize = b.option(std.builtin.OptimizeMode, "wasm-optimize", "Optimize mode for wasm build") orelse optimize;
    ...
    const install_wasm = b.addInstallArtifact(wasm_exe, .{
        .dest_dir = .{ .override = .{ .custom = "../lib/wasm" } },
    });
